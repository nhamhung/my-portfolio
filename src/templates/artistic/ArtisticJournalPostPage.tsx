...
const artisticMarkdown: Components = {
  h1: ({ children }) => (
    <Text
      as="h2"
      className="artistic-article-heading artistic-article-heading-2"
    >
      {children}
    </Text>
  ),
  h2: ({ children }) => (
    <Text
      as="h3"
      className="artistic-article-heading artistic-article-heading-3"
    >
      {children}
    </Text>
  ),
  h3: ({ children }) => (
    <Text
      as="h4"
      className="artistic-article-heading artistic-article-heading-4"
    >
      {children}
    </Text>
  ),
...
function ArtisticJournalPostPage({ slug }: JournalPostPageProps) {
  const post = getLocalJournalPostBySlug(slug);
  return (
    <Box
      className="artistic-publication-page"
      data-presentation="artistic-journal-post"
    >
      <Link
        href="#/journal"
        className="artistic-article-back"
      ...
      {!post ? (
        <Box
          className="artistic-article-not-found"
          data-testid="journal-post-not-found"
        >
          <Text>Journal / 404</Text>
        ...
        <Box
          as="article"
          className="artistic-publication-article"
          data-testid="journal-post-page"
        >
          <Box as="figure" className="artistic-article-hero">
            <Image
              src={post.image}
              alt={post.imageAlt}
          ...
          <Box as="header" className="artistic-article-header">
            <Text>{post.topics.join(" · ")}</Text>
            <Text as="h1">{post.title}</Text>
            <Text>{post.summary}</Text>
          </Box>
          <Box className="artistic-article-body">
            <ReactMarkdown components={artisticMarkdown}>
              {post.content}
            </ReactMarkdown>
          </Box>
        </Box>
...
export default ArtisticJournalPostPage;
